...
[System.Serializable]
public class XnXMLReader {
	static public bool		SHOW_COMMENTS = false;

	public string xmlText;
	public XnXMLHashtable xml;
	
	public void Parse(string eS) {
		xmlText = eS;
		xml = new XnXMLHashtable();
		Parse(eS, xml);
	}
	
	void Parse(string eS, XnXMLHashtable eH) {
		eS = eS.Trim();
		while(eS.Length > 0) {
			eS = ParseTag(eS, eH);
			eS = eS.Trim();
...
public class XnXMLHashList {
	public ArrayList list = new ArrayList();
	
	public XnXMLHashtable this[int s] {
		get {
			return(list[s] as XnXMLHashtable);
		}
		set {
			list[s] = value;
		}
	...
	public void Add(XnXMLHashtable eH) {
		list.Add(eH);
	}
...
public class XnXMLHashtable {
	
	public List<string>				keys = new List<string>();
	public List<XnXMLHashList>		nodesList = new List<XnXMLHashList>();
	public List<string>				attKeys = new List<string>();
	public List<string>				attributesList = new List<string>();
	
	public XnXMLHashList Get(string key) {
		int ndx = Index(key);
	...
	public XnXMLHashList this[string s] {
		get {
			return( Get(s) );
		}
		set {
